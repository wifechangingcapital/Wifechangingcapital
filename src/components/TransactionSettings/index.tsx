...
export default function TransactionSettings({ placeholderSlippage }: TransactionSettingsProps) {
  ...
  function parseSlippageInput(value: string) {
    ...
    if (value.length === 0) {
      ...
      ///Below is where you change the max slippage settings if parsed output <0 set to auto, if above 12,000 (ie 120%) throw error

      if (!Number.isInteger(parsed) || parsed < 0 || parsed > 12000) {
        setUserSlippageTolerance('auto')
        if (value !== '.') {
  ...
  // const tooHigh = userSlippageTolerance !== 'auto' && userSlippageTolerance.greaterThan(new Percent(0, 10))
